...
bool SearchBarLineEdit::eventFilter(QObject *, QEvent *event)
{
    ...
    {
        auto key = e->key();
        bool isScrollDownKey = key == Qt::Key_Down || key == Qt::Key_PageDown;
        bool noModifiers = e->modifiers().testFlag(Qt::NoModifier);
...
void SearchBarLineEdit::onScroll(int value)
{
    ...
    /* Scrolling using key_down past end will teleport scroller to the top.
       We undo this here. Block signal to avoid recursion.
    */
...
void SearchBarLineEdit::onInitialSuggestions(int)
{
    if (m_returnPressed) {
        ...
        auto completerFirstIdx = m_suggestionView->model()->index(0, 0);
        auto completerSelModel = m_suggestionView->selectionModel();
...
void SearchBarLineEdit::onAdditionalSuggestions(int start)
{
    ...
    auto completerStartIdx = m_suggestionView->model()->index(start, 0);
...
{
    auto firstSuggestionIndex = m_suggestionModel.index(0);
    ...
    auto firstSuggestionText = firstSuggestionIndex.data().toString();
