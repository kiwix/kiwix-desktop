    ...
    , ui(new Ui::Settings)
{
    ...
    connect(ui->downloadDirPathCopy, &QPushButton::clicked, [this]() {
        copySettingsPathToClipboard(ui->downloadDirPathCopy);
    });
    connect(ui->monitorDirPathCopy, &QPushButton::clicked, [this]() {
        copySettingsPathToClipboard(ui->monitorDirPathCopy);
    });
...
void SettingsView::copySettingsPathToClipboard(QPushButton* button)
{
    auto settingsManager = KiwixApp::instance()->getSettingsManager();
    QString pathToCopy = (button == ui->downloadDirPathCopy) 
        ? settingsManager->getDownloadDir() 
        : settingsManager->getMonitorDir();
